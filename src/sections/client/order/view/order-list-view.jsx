...
const STATUS_OPTIONS = [{ value: 'all', label: 'Todas' }, ...ORDER_STATUS_OPTIONS];
...
export function OrderListView() {
  ...
  const handleViewRow = useCallback(
    (id) => {
      router.push(paths.client.orders.details(id));
    },
    [router]
  );
  ...
  return (
    <>
      <Box maxWidth="xl" sx={{ p: 3 }}>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'Órdenes', href: paths.client.orders.root },
            { name: 'Listado' },
          ]}
          sx={{ mb: { xs: 3, md: 5 } }}
        />
      ...
      <ConfirmDialog
        ...
        content={
          <>
            ¿Estás seguro de que deseas eliminar <strong> {table.selected.length} </strong> elementos?
          </>
        }
        action={
          <Button
